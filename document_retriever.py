...
SITE_CONFIGS = [
    {
        ...
        "selectors": {"title": "h1, .title", "content": ".content, article, .detail", "date": ".date, time"}
    },
]
...
class DocumentRetriever:
    ...
    def __init__(self, embeddings, knowledge_base=None, site_configs=None, serpapi_key=None):
        """
        Args:
    ...
    def _build_search_query(self, claim: str) -> str:
        ...
        stop_words = {'là', 'của', 'và', 'có', 'này', 'được', 'đã', 'trong', 'cho',
                     'về', 'với', 'một', 'các', 'trên', 'từ', 'đến', 'sau', 'bị', 'cũng',
                     'như', 'khi', 'vào', 'ra', 'để', 'theo', 'sẽ', 'bởi', 'tại'}
        words = re.findall(r'\b\w+\b', claim_lower)
        keywords = [word for word in words if len(word) > 2 and word not in stop_words]
        return ' '.join(keywords[:5])

    def _extract_article_links(self, html: str, base_domain: str, config: Dict, max_links: int = 30) -> List[str]:
    ...
    def _search_google(self, query: str, domain: str, max_results: int = 10) -> List[str]:
    ...
    async def _crawl_articles(self, claim: str, max_per_site: int = 5) -> List[Dict]:
        ...
        async with AsyncWebCrawler(browser_type="chromium", headless=True, verbose=False) as crawler:
            for config in self.site_configs:
                ...
                try:
                    ...
                    article_urls = self._extract_article_links(
                        search_result.html, config['site'], config, max_links=max_per_site * 2
                    )
                    print(f"  Found {len(article_urls)} result links")

                    if len(article_urls) == 0 and self.serpapi_key:
                        print("  🔄 Trying Google search fallback...")
                        article_urls = self._search_google(query, config['site'], max_results=max_per_site * 2)
                    ...
                    for url in article_urls:
                        ...
                        try:
                            ...
                            content = self._extract_content(result.html, config['selectors'])
                            if not content.get('content') and result.markdown:
                                content['content'] = result.markdown[:3000]
                                content['title'] = result.markdown.split('\n')[0][:80]
    ...
    async def retrieve_for_all_claims(self, claims: List[Dict[str, Any]], k: int = 10) -> Dict[int, List[Document]]:
        """
